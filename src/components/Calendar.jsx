...
const DEFAULT_CATS = [
  { id: 'work',     name: 'עבודה',   color: '#3b82f6' },
  { id: 'personal', name: 'אישי',    color: '#10b981' },
  { id: 'health',   name: 'בריאות',  color: '#ef4444' },
  { id: 'study',    name: 'לימודים', color: '#8b5cf6' },
];
...
// ── Live "now" position (updates every 30 s) ──────────────────────────────────
function useNowTop() {
  const calc = () => {
    const n = new Date();
    return (getHours(n) * 60 + getMinutes(n)) / 60 * HOUR_HEIGHT;
  };
  const [top, setTop] = useState(calc);
  useEffect(() => {
    const id = setInterval(() => setTop(calc()), 30_000);
...
export default function Calendar({ events, tasks, onCreateEvent, onUpdateEvent, onDeleteEvent, onCreateTask, onUpdateTask }) {
  ...
  const titleLabel = () => {
    if (view === 'month') return format(anchor, 'MMMM yyyy', { locale: he });
    if (view === 'week') {
      const ws = startOfWeek(anchor, { weekStartsOn: 0 });
      const we = endOfWeek(anchor, { weekStartsOn: 0 });
      return isSameMonth(ws, we)
        ? format(ws, 'MMMM yyyy', { locale: he })
        : `${format(ws, 'MMM', { locale: he })} – ${format(we, 'MMM yyyy', { locale: he })}`;
    }
    return format(anchor, 'EEEE, d MMMM yyyy', { locale: he });
  };
  ...
  const handleFormSubmit = (e) => {
    ...
    const payload = { ...formData, startDate: formData.startDate + 'T00:00:00', endDate: (formData.endDate || formData.startDate) + 'T00:00:00' };
  ...
  const submitTaskDraft = (e) => {
    ...
    onCreateTask?.({ title: taskDraft.title.trim(), status:'todo', priority:'medium', date: taskDraft.date + 'T00:00:00' });
    setTaskDraft(null);
  };
  ...
  const dayEvents = (day) => visibleEvents.filter(ev => ev.startDate && isSameDay(parseISO(ev.startDate), day));
  const dayTasks  = (day) => tasks.filter(t => t.date && isSameDay(parseISO(t.date), day));
    ...
    <div className="gcal">
      ...
      <div className="gcal__topbar">
        <div className="gcal__topbar-left">
          <button className="gcal__today-btn" onClick={() => setAnchor(new Date())}>היום</button>
          ...
          <div className="gcal__cats">
            ...
            {showAddCat ? (
              <div className="gcal__cat-new-form">
                <input autoFocus placeholder="שם קטגוריה" value={newCatName} onChange={e => setNewCatName(e.target.value)} onKeyDown={e => { if (e.key === 'Enter') addCategory(); if (e.key === 'Escape') setShowAddCat(false); }} />
              ...
              <button className="gcal__cat-add" onClick={() => setShowAddCat(true)}>+ קטגוריה</button>
            )}
          </div>
        </div>
        ...
        <div className="gcal__task-overlay" onClick={() => setTaskDraft(null)}>
          <form className="gcal__task-form" onClick={e=>e.stopPropagation()} onSubmit={submitTaskDraft}>
            <div className="gcal__task-form-head">
              <span>✅ משימה חדשה ל-{taskDraft.date}</span>
            ...
            <div className="gcal__task-form-foot">
              <button type="button" className="gcal__task-form-cancel" onClick={() => setTaskDraft(null)}>ביטול</button>
              <button type="submit" className="gcal__task-form-submit">הוסף משימה</button>
            </div>
          </form>
        </div>
      )}
...
function MonthView({ anchor, dayEvents, dayTasks, onDayClick, onEventClick, setAnchor, setView, categories, onUpdateEvent, onUpdateTask, onAddTask }) {
  ...
  return (
    <div className="gcal__month">
      <div className="gcal__month-header">{DAY_NAMES_SHORT.map(d => <div key={d} className="gcal__month-dow">{d}</div>)}</div>
          ...
          return (
            ...
            >
              ...
              <div className="gcal__month-cell-events">
                ...
                {(evs.length + ts.length) > 4 && <div className="gcal__month-more">+{evs.length + ts.length - 4} עוד</div>}
              </div>
            </div>
          );
        })}
  ...
  return (
    <div className="gcal__week">
      <div className="gcal__week-header">
        ...
        {days.map((day, i) => (
          <div key={i} className={`gcal__week-day-header ${isToday(day) ? 'today' : ''}`}>
            <span className="gcal__week-dow">{DAY_NAMES_SHORT[i]}</span>
      ...
      <div className="gcal__allday-row">
        <div className="gcal__time-gutter gcal__allday-label">כל היום</div>
        {days.map((day, i) => {
          ...
          return (
            <div key={i} className="gcal__allday-cell">
              ...
              <button className="gcal__add-task-btn" onClick={() => onAddTask?.(day)} title="הוסף משימה">+ משימה</button>
            </div>
          );
        })}
      </div>
          ...
          {days.map((day, di) => (
            <div
              ...
              onClick={e => {
                ...
                onSlotClick(day, minutesToTime(Math.round((y / HOUR_HEIGHT * 60) / 30) * 30));
              }}
            ...
            >
              {isToday(day) && (
                <div className="gcal__now-line" style={{ top: nowTop }}>
                  <div className="gcal__now-dot" /><div className="gcal__now-bar" />
                </div>
              )}
    ...
    <div className="gcal__week gcal__day">
      <div className="gcal__week-header">
        ...
        <div className={`gcal__week-day-header gcal__week-day-header--full ${isToday(anchor) ? 'today' : ''}`}>
          <span className="gcal__week-dow">{DAY_NAMES_HE[anchor.getDay()]}</span>
      ...
      <div className="gcal__allday-row">
        ...
        <div className="gcal__allday-cell" style={{ flex:1 }}>
          ...
          <button className="gcal__add-task-btn" onClick={onAddTask} title="הוסף משימה">+ משימה</button>
        </div>
      </div>
      <div
        className="gcal__time-grid-wrap"
      ...
      >
        <div className="gcal__time-grid" style={{ height: HOUR_HEIGHT * 24 }}>
          ...
          <div className="gcal__day-col" style={{ left:'60px', width:'calc(100% - 60px)' }}
            onClick={e => { if (dragging) return; const rect = e.currentTarget.getBoundingClientRect(); const y = e.clientY - rect.top + (gridWrapRef.current?.scrollTop||0); onSlotClick(minutesToTime(Math.round((y/HOUR_HEIGHT*60)/30)*30)); }}
            onDragOver={e => { if (e.dataTransfer.types.includes('cal-task-id')) e.preventDefault(); }}
            onDrop={e => {
              e.preventDefault();
              const taskId = e.dataTransfer.getData('cal-task-id');
            ...
            {isToday(anchor) && <div className="gcal__now-line" style={{ top: nowTop }}><div className="gcal__now-dot"/><div className="gcal__now-bar"/></div>}
            {dragging && (() => {
...
const STATUS_LABEL   = { backlog:'Backlog', todo:'To Do', inprogress:'In Progress', review:'Review', done:'הושלם' };
...
function TasksPanel({ tasks, onUpdateTask, onClose }) {
    ...
    <div className="gcal__tp-section">
      ...
      {items.map(t => (
        <div key={t.id} className={`gcal__tp-task${t.status==='done'?' gcal__tp-task--done':''}`}
          ...
          {t.date && (
            <button className="gcal__tp-unschedule" title="הסר תאריך"
  ...
  return (
    <div className="gcal__tasks-panel">
      ...
      <p className="gcal__tp-hint">גרור משימה לזמן ביומן כדי לתזמן אותה</p>
      <div className="gcal__tp-list">
        <Section title="ללא תאריך" items={unscheduled} />
        <Section title="מתוזמנות" items={scheduled} />
        <Section title="הושלמו" items={done} />
        {tasks.filter(t => t.status !== 'done').length === 0 && (
          <div className="gcal__tp-empty">כל המשימות הושלמו 🎉</div>
        )}
      </div>
    </div>
  );
    ...
    <div className="gcal__modal-overlay" onClick={onClose}>
      <div className="gcal__modal" onClick={e => e.stopPropagation()}>
        <div className="gcal__modal-header">
          <h3>{editingEvent ? 'עריכת אירוע' : 'אירוע חדש'}</h3>
        ...
        <form onSubmit={onSubmit} className="gcal__modal-body">
          <input className="gcal__modal-title-input" type="text" placeholder="הוסף כותרת" value={formData.title} onChange={e => set('title', e.target.value)} autoFocus required />
          ...
          <div className="gcal__modal-field gcal__modal-allday">
            <label className="gcal__toggle-wrap">
              ...
              <span className="gcal__toggle" /><span>כל היום</span>
            </label>
          </div>
          ...
          {categories.length > 0 && (
            <div className="gcal__modal-field">
              ...
              <div className="gcal__modal-cats">
                <button type="button" className={`gcal__modal-cat ${!formData.categoryId ? 'active' : ''}`} onClick={() => set('categoryId', null)}>ללא קטגוריה</button>
                {categories.map(c => (
          ...
          <div className="gcal__modal-field">
            ...
            <textarea placeholder="הוסף תיאור..." value={formData.description} onChange={e => set('description', e.target.value)} rows={2} />
          </div>
          ...
          <div className="gcal__modal-footer">
            {editingEvent && <button type="button" className="gcal__modal-delete" onClick={onDelete}><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><polyline points="3 6 5 6 21 6"/><path d="M19 6l-1 14H6L5 6"/></svg>מחק</button>}
            <div style={{ marginRight:'auto', display:'flex', gap:'8px' }}>
              <button type="button" className="gcal__modal-cancel" onClick={onClose}>ביטול</button>
              <button type="submit" className="gcal__modal-save">שמור</button>
            </div>
          </div>
        </form>
      </div>
